...
<section>
					...
					<div class="mainImage">
						<c:forEach var="af" items="${ad.acFiles }">
							<c:if test="${fn:contains(af.afMain,'Y')}">
								<img src="${path}/images/upload/accommodation/${af.afName}"alt="" />
							</c:if>
						</c:forEach>
					</div>
				...
				<div class="subImageCon">
					<div class="subImageLine firstline">
						<c:forEach var="af" items="${ad.acFiles }" varStatus="i">
							<c:if test="${fn:contains(af.afMain,'N') and i.index<3 and i.index>0}">
								<div class="subImage">
									<img src="${path}/images/upload/accommodation/${af.afName}"alt="" />
								</div>
							</c:if>
				...
				<div class="detailHotelModalCon">
					<div class="detailHotelModal">
						<div class="detailHotelPrice">
							<div>
								<span> ₩<fmt:formatNumber value="${ad.acPrice }" pattern="#,###" /> <span> /박</span></span>
							</div>
						</div>
						...
						<div class="detailHotelPriceCalc">
							<div>
								<div class="detailHotelEx">
									<span id="exPrice">₩<fmt:formatNumber value="${ad.acPrice }" pattern="#,###" /></span> <span>X</span> <span
										id="exDay">1박</span>
								</div>
								<span id="resultPrice">₩<fmt:formatNumber value="${ad.acPrice }" pattern="#,###" /></span>
							</div>
						</div>
						<div class="hrCon">
							<hr />
						...
						<div class="detailHotelPriceResult">
							<div>
								<span>총 합계</span> <span id="realResultPrice">₩<fmt:formatNumber value="${ad.acPrice }" pattern="#,###" /></span>
							</div>
						</div>
					</div>
				</div>
	...
	geocoder.addressSearch(addr, function(result, status) {
		if (status === kakao.maps.services.Status.OK) {
			...
			var staticMapContainer  = document.getElementById('mapContainer'), // 이미지 지도를 표시할 div  
		    staticMapOption = { 
